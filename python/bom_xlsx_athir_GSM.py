...
def myEqu(self, other):
    ...
    if self.getValue() != other.getValue():
        ...
        result = False
    elif self.getField("GSM-Mounting") != other.getField("GSM-Mounting"):
        result = False
    elif self.getField("None-GSM-Mounting") != other.getField("None-GSM-Mounting"):
        result = False
        #print(other.getValue())
...
worksheet.append(['Components:', "=SUM(B7:B150)-SUM.HVIS(H7:H150,\"-\",B7:B150)"] )
#worksheet.append(['Reels',reels])
#worksheet.append(['Reel Count:', reels] )
worksheet.append(['Ref', 'Quantity', 'Value', 'Footprint', 'Type','Tolerance','Voltage','GSM-Mounting','None-GSM-Mounting','Manufacturer','Comment'] )#, 'Comment', 'Datasheet'
...
for group in grouped:
    ...
    if(excluded(exvals,c)):
        worksheet.append([refs, len(group), c.getValue(), footprint, c.getField("Type"), c.getField('Tolerance'), c.getField('Voltage'), c.getField("GSM-Mounting"), c.getField("None-GSM-Mounting"), c.getField("Manufacturer"), c.getField("Comment")]) #, c.getDatasheet()
...
for cells in worksheet["B7":"J150"]:
    for cell in cells:
        cell.style = 'centered'

for cells in worksheet["H7":"I150"]:
    for cell in cells:
        if(re.search("-",str(cell.value))):
            cell.fill = redFill
        elif(re.search("Bottom",str(cell.value))):
            cell.fill = greenFill
        elif(re.search("Not Mounted",str(cell.value))):
            cell.fill = redFill

for cells in worksheet["K7":"K150"]:
    for cell in cells:
        cell.alignment = Alignment(horizontal="right")

for cells in worksheet["A7":"K150"]:
    if(cells[0].value != None):
        #print(str(cells[0].value))
        for cell in cells:
            cell.border = Border(bottom=thin)

for cells in worksheet["A7":"A150"]:
	if(cells[0].value != None):
		for cell in cells:
...
worksheet.column_dimensions["A"].width = getColumnWidth(worksheet["A6":"A150"])
worksheet.column_dimensions["B"].width = getColumnWidth(worksheet["B6":"B150"])
worksheet.column_dimensions["C"].width = getColumnWidth(worksheet["C6":"C150"])
worksheet.column_dimensions["D"].width = getColumnWidth(worksheet["D6":"D150"])
worksheet.column_dimensions["E"].width = getColumnWidth(worksheet["E6":"E150"])
worksheet.column_dimensions["F"].width = getColumnWidth(worksheet["F6":"F150"])
worksheet.column_dimensions["G"].width = getColumnWidth(worksheet["G6":"G150"])
worksheet.column_dimensions["H"].width = getColumnWidth(worksheet["H6":"H150"])
worksheet.column_dimensions["I"].width = getColumnWidth(worksheet["I6":"I150"])
worksheet.column_dimensions["J"].width = getColumnWidth(worksheet["J6":"J150"])
worksheet.column_dimensions["K"].width = getColumnWidth(worksheet["K6":"K150"])
